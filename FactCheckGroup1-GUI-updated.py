...
def results(y_pred, data_sets):
    ...
    sb.heatmap(cm, vmin=0, vmax=100, annot=True, ax=ax)
...
def decision_tree():
    ...
    if data_sets is None:
        return
    tree = DecisionTreeClassifier().fit(data_sets['x_train'], data_sets['y_train'])
    y_pred = tree.predict(data_sets['x_test'])
    results(y_pred, data_sets)

# initialize UI window
window = Tk()
window.title("Make Facts Great Again")
...
group_train = LabelFrame(window, text="Choose Training Datasets", font='Calibri 12 bold', height=150, width=320, padx=5, pady=5)
...
group_train_text.grid(row=8, column=0, padx=5, pady=5, sticky=W)
...
group_test = LabelFrame(window, text="Choose Test Datasets", font= 'Calibri 12 bold', height=260, width=460, padx=5, pady=5)
